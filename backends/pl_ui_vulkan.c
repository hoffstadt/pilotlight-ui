...
static void                   pl__grow_vulkan_vertex_buffer   (uint32_t uVtxBufSzNeeded, plVulkanBufferInfo* ptBufferInfo);
static void                   pl__grow_vulkan_index_buffer    (uint32_t uIdxBufSzNeeded, plVulkanBufferInfo* ptBufferInfo);
...
pl_initialize_vulkan(const plVulkanInit* ptInit)
{
    ...
    ptVulkanDrawContext->uFramesInFlight = ptInit->uFramesInFlight;
    ptCtx->tIO.pBackendRendererData = ptVulkanDrawContext;
    ...
    assert(vkCreateShaderModule(ptVulkanDrawContext->tDevice, &tVtxShdrInfo, NULL, &ptVulkanDrawContext->tVtxShdrStgInfo.module) == VK_SUCCESS);
    ...
    assert(vkCreateShaderModule(ptVulkanDrawContext->tDevice, &tPxlShdrInfo, NULL, &ptVulkanDrawContext->tPxlShdrStgInfo.module) == VK_SUCCESS);
...
pl_submit_vulkan_drawlist_ex(plDrawList* ptDrawlist, float fWidth, float fHeight, VkCommandBuffer tCmdBuf, uint32_t uFrameIndex, VkRenderPass tRenderPass, VkSampleCountFlagBits tMSAASampleCount)
{
    ...
    if(uVtxBufSzNeeded >= uAvailableVertexBufferSpace)
        pl__grow_vulkan_vertex_buffer(uVtxBufSzNeeded * 2, tBufferInfo);
    ...
    if(uIdxBufSzNeeded >= uAvailableIndexBufferSpace)
        pl__grow_vulkan_index_buffer(uIdxBufSzNeeded * 2, tBufferInfo);
